...
export function OverflowContainer<T>({
  ...
  useLayoutEffect(() => {
    ...
    const observer = new ResizeObserver(entries => {
      ...
      let amount = 0 // Overflow ammount is null / 0
      for (let i = children.length - 1; i >= 0; i--) {
        const child = children[i]
        if (containerElement.scrollHeight <= containerElement.clientHeight) { // Condition to Check Overflow
          break
        }
        amount = children.length - i // changing Overflow
        child.style.display = "none"  // Hiding Element if Overflow
